...
function _redesignTripCards() {
  ...
  Array.from(list.querySelectorAll('.trip-card')).forEach(function(card) {
    // Evitar reprocesar cards ya rediseñadas (tienen .trip-bus-icon)
    if (card.querySelector('.trip-bus-icon')) return;
...
function _redesignHomeCards() {
  var cardsEl = document.querySelector('#view-home .cards, #view-home .action-cards');
  if (!cardsEl) return;
...
function _redesignFloorCards() {
  ...
  // Siempre convertir el contenedor
  ...
  // Procesar solo las .card que aún no fueron rediseñadas
  // (los nodos nuevos de app.js no tienen data-redesigned)
  Array.from(box.querySelectorAll('.card')).forEach(function(card, i) {
    ...
    if (title.toLowerCase().indexOf('mirá') >= 0 ||
        ...
        title.toLowerCase().indexOf('ver')  >= 0) {
      iconKey = 'find';
      // Normalizar texto igual al convencional
      title = 'Ver mi asiento';
      desc  = 'Ingresá tu documento para encontrar tu número y ubicación.';
    }
