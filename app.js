...
function goFind(){ if(!CURRENT_TRIP.fileId){ setHash(['Inicio']); showView('view-choose'); return; } showView('view-find'); clearFindView(); clearFindViewMulti(); setHash(['Mira-tu-asiento', CURRENT_TRIP.name]); } 
...
async function confirmReservationPage(){
  ...
  if (selected.size === 1) {
    ...
    _markField(nameEl, !name);
    _markField(ciEl,   !ci);

    if (!name || !ci) {
      if (!name && nameEl) firstInvalid = nameEl;
      else if (!ci && ciEl) firstInvalid = ciEl;
    ...
    // ── Múltiple — leer desde .reserve-card[data-code] ──
    ...
    cards.forEach(function(card) {
      ...
      if (!name || !ci) {
        invalid = true;
        if (!firstInvalid) firstInvalid = (!name && nameEl) ? nameEl : ciEl;
      }

      pairs.push({
        asiento  : card.dataset.code,
        pasajero : name,
        ci       : ci
      });
    });
...
/* Marca / desmarca un campo como inválido visualmente */
function _markField(el, isInvalid) {
  if (!el) return;
  if (isInvalid) {
